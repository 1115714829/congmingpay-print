// Package fingerprint 采集本机硬件指纹并计算与 web 管理端一致的身份摘要。
// canonical 规则(两端必须字节一致):只含 4 个身份字段、按名字典序、
// TrimSpace、diskSerials 排序后数组输出;摘要 SHA256。见 web/api/API.md。
package fingerprint
...
// Fingerprint 是上报管理端(web /device/lookup、/device/bind)的硬件指纹。
// 参与摘要(身份)的字段必须跨重装不变;展示字段不参与摘要。
// 不含网卡 MAC(网卡不稳定、USB 网卡干扰);DiskSerials 只含系统盘(避免多盘干扰)。
type Fingerprint struct {
	OsType      string   `json:"osType"`      // "win"(参与摘要)
...
// 与 web/backend/internal/fingerprint 的 Hash 实现保持逐字节一致。
