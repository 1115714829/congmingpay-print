...
- **系统设置**:服务名(**用作窗口标题与托盘提示**,保存即时生效、空回退默认) + **系统通知开关**(`NotifyDisabled` 反向字段,默认开,保存即时生效) + **云端 MQTT**(Tab 二选一:`generic` 自建 · `aliyun` 云消息队列 AK/SK+`Group@@@商户号`+父主题/上下行后缀自动拼;`Resolve`统一产出;**启用时校验当前 Tab**) + **本地接口文档服务**(启用/端口/局域网地址,只读文档) + 保存。
- **新增打印机三步向导** + **属性对话框**。
...
> 云端通道 = **MQTT(唯一,已实现,`internal/mqtt`)**:Provider 二选一——**自建**(用户名密码,订阅短商户号,上报手填主题)或**阿里云云消息队列 MQTT 版**(AK/SK 签名;`GroupId@@@自定义ID`;主题 `{父主题}/{自定义ID}/{上下行后缀}`;接入点手填)。payload=`PrintRequest` 单/数组收打印;上行 **report** / **state**(定时+查询+LWT);每条带 `merchant`(自建=短商户号,阿里云=自定义ID)。**数据 HTTP 通道已移除**;另有 `internal/docserver` 只读接口文档页。
